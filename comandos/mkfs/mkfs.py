...
# //////////////////////////////////////
def crear_remover_gu(sblock, inodo_archivo, i_ino, txt):
    segmentSize = 64
    totalSegments = (len(txt)) // segmentSize
    ...
    totalSegments -= 12
    if totalSegments > 0:
        if inodo_archivo.i_block[12] == -1:
            if sblock.s_first_blo == -1:
                # Ya no hay suficientes bloques
                print("Error: Ya no hay suficientes bloques")
                file.close()
                return
            inodo_archivo.i_block[12] = sblock.s_first_blo
            write_on_inodo = sblock.s_inode_start + (ctypes.sizeof(structs.Inodo) * i_ino)
            file.seek(write_on_inodo)
            file.write(ctypes.string_at(ctypes.byref(inodo_archivo), ctypes.sizeof(inodo_archivo)))

            bloque_s_indirecto = structs.BloqueApuntadores()
            write_on_block = sblock.s_block_start + (ctypes.sizeof(structs.BloqueApuntadores) * inodo_archivo.i_block[12])
            file.seek(write_on_block)
            file.write(ctypes.string_at(ctypes.byref(bloque_s_indirecto), ctypes.sizeof(bloque_s_indirecto)))

            write_on_block = sblock.s_bm_block_start + inodo_archivo.i_block[12]
            file.seek(write_on_block)
            file.write(b's')
        ...
        for s in range(iterations_blocks):
            ...
            if bloque_s_indirecto.b_pointers[s] == -1:
                if sblock.s_first_blo == -1:
                    ...
                    file.close()
                    return
                bloque_s_indirecto.b_pointers[s] = sblock.s_first_blo
                write_on_b = sblock.s_bm_block_start + sblock.s_first_blo
                file.seek(write_on_b)
                file.write(b'f')
                sblock.s_first_blo = next_first_block(sblock, session_inciada.mounted.path)
            ...
            write_on = sblock.s_block_start + (ctypes.sizeof(structs.BloqueArchivo) * bloque_s_indirecto.b_pointers[s])
            file.seek(write_on)
            file.write(file_user)

        write_on = sblock.s_block_start + (ctypes.sizeof(structs.BloqueApuntadores) * inodo_archivo.i_block[12])
        file.seek(write_on)
        file.write(ctypes.string_at(ctypes.byref(bloque_s_indirecto), ctypes.sizeof(bloque_s_indirecto)))

        # write_on_b = sblock.s_bm_block_start + inodo_archivo.i_block[12]
        # file.seek(write_on_b)
        # file.write(b's')
    
    totalSegments -= 16
    if totalSegments > 0:
        if inodo_archivo.i_block[13] == -1:
            if sblock.s_first_blo == -1:
                # Ya no hay suficientes bloques
                print("Error: Ya no hay suficientes bloques")
                file.close()
                return
            inodo_archivo.i_block[13] = sblock.s_first_blo
            write_on_inodo = sblock.s_inode_start + (ctypes.sizeof(structs.Inodo) * i_ino)
            file.seek(write_on_inodo)
            file.write(ctypes.string_at(ctypes.byref(inodo_archivo), ctypes.sizeof(inodo_archivo)))

            bloque_d_indirecto = structs.BloqueApuntadores()
            write_on_block = sblock.s_block_start + (ctypes.sizeof(structs.BloqueApuntadores) * inodo_archivo.i_block[13])
            file.seek(write_on_block)
            file.write(ctypes.string_at(ctypes.byref(bloque_d_indirecto), ctypes.sizeof(bloque_d_indirecto)))

            write_on_block = sblock.s_bm_block_start + inodo_archivo.i_block[13]
            file.seek(write_on_block)
            file.write(b'l')
        ...
        bloque_d_indirecto = structs.BloqueApuntadores()
        read_on_block = sblock.s_block_start + (ctypes.sizeof(structs.BloqueApuntadores) * inodo_archivo.i_block[13])
        file.seek(read_on_block)
        file.readinto(bloque_d_indirecto)
        ...
        for d in range(iterations_db):
            bloque_s_indirecto = structs.BloqueApuntadores()
            if bloque_d_indirecto.b_pointers[d] == -1:
                bloque_d_indirecto.b_pointers[d] = sblock.s_first_blo
                
                write_on_block = sblock.s_bm_block_start + bloque_d_indirecto.b_pointers[d]
                file.seek(write_on_block)
                file.write(b's')

                write_on_block = sblock.s_block_start + (ctypes.sizeof(structs.BloqueArchivo) * inodo_archivo.i_block[13])
                file.seek(write_on_block)
                file.write(ctypes.string_at(ctypes.byref(bloque_d_indirecto), ctypes.sizeof(bloque_d_indirecto)))
                sblock.s_first_blo = next_first_block(sblock, session_inciada.mounted.path)
                sblock.s_free_blocks_count -= 1
            else:
                read_on_block = sblock.s_block_start + (ctypes.sizeof(structs.BloqueApuntadores) * bloque_d_indirecto.b_pointers[d])
                file.seek(read_on_block)
                file.readinto(bloque_s_indirecto)
            for s in range(16):
                new_segment = txt[(s + 28 + (d * 16)) * segmentSize : (s + 29 + (d * 16)) * segmentSize]
            ...
            write_on = sblock.s_block_start + (ctypes.sizeof(structs.BloqueApuntadores) * bloque_d_indirecto.b_pointers[d])
            file.seek(write_on)
            file.write(ctypes.string_at(ctypes.byref(bloque_s_indirecto), ctypes.sizeof(bloque_s_indirecto)))

    # Tercer bloque indirecto
    if totalSegments > 0:
        if inodo_archivo.i_block[14] == -1:
            if sblock.s_first_blo == -1:
                # Ya no hay suficientes bloques
                print("Error: Ya no hay suficientes bloques")
                file.close()
                return
            inodo_archivo.i_block[14] = sblock.s_first_blo
            write_on_inodo = sblock.s_inode_start + (ctypes.sizeof(structs.Inodo) * i_ino)
            file.seek(write_on_inodo)
            file.write(ctypes.string_at(ctypes.byref(inodo_archivo), ctypes.sizeof(inodo_archivo)))

            bloque_d_indirecto = structs.BloqueApuntadores()
            write_on_block = sblock.s_block_start + (ctypes.sizeof(structs.BloqueApuntadores) * inodo_archivo.i_block[14])
            file.seek(write_on_block)
            file.write(ctypes.string_at(ctypes.byref(bloque_d_indirecto), ctypes.sizeof(bloque_d_indirecto)))

            write_on_block = sblock.s_bm_block_start + inodo_archivo.i_block[14]
            file.seek(write_on_block)
            file.write(b't')
        ...
        bloque_t_indirecto = structs.BloqueApuntadores()
        read_on_block = sblock.s_block_start + (ctypes.sizeof(structs.BloqueApuntadores) * inodo_archivo.i_block[14])
        file.seek(read_on_block)
        file.readinto(bloque_t_indirecto)
        ...
        for t in range(iterations_tb):
            bloque_d_indirecto = structs.BloqueApuntadores()
            if bloque_t_indirecto.b_pointers[t] == -1:
                bloque_t_indirecto.b_pointers[t] = sblock.s_first_blo
                
                write_on_block = sblock.s_bm_block_start + bloque_t_indirecto.b_pointers[t]
                file.seek(write_on_block)
                file.write(b'l')

                write_on_block = sblock.s_block_start + (ctypes.sizeof(structs.BloqueArchivo) * inodo_archivo.i_block[14])
                file.seek(write_on_block)
                file.write(ctypes.string_at(ctypes.byref(bloque_t_indirecto), ctypes.sizeof(bloque_t_indirecto)))
                sblock.s_first_blo = next_first_block(sblock, session_inciada.mounted.path)
                sblock.s_free_blocks_count -= 1
            else:
                read_on_block = sblock.s_block_start + (ctypes.sizeof(structs.BloqueApuntadores) * bloque_t_indirecto.b_pointers[t])
                file.seek(read_on_block)
                file.readinto(bloque_d_indirecto)
            ...
            for d in range(16):
                ...
                if bloque_d_indirecto.b_pointers[d] == -1:
                    ...
                    write_on_block = sblock.s_block_start + (ctypes.sizeof(structs.BloqueArchivo) * bloque_t_indirecto.b_pointers[t])
                    file.seek(write_on_block)
                    file.write(ctypes.string_at(ctypes.byref(bloque_d_indirecto), ctypes.sizeof(bloque_d_indirecto)))
                    sblock.s_first_blo = next_first_block(sblock, session_inciada.mounted.path)
                    sblock.s_free_blocks_count -= 1
            ...
            write_on = sblock.s_block_start + (ctypes.sizeof(structs.BloqueApuntadores) * bloque_t_indirecto.b_pointers[t])
            file.seek(write_on)
            file.write(ctypes.string_at(ctypes.byref(bloque_d_indirecto), ctypes.sizeof(bloque_d_indirecto)))
...
def crear_grupo_usuario(sblock, data, name_g_u, tipo, user_actual = None):
    ...
    txt += data
    crear_remover_gu(sblock, inodo_archivo, i_ino, txt)
...
def make_ext3(part_size, start, path):
    ...
    super_bloque.s_bm_inode_start = start + ctypes.sizeof(super_bloque) + (numero_estructuras * ctypes.sizeof(structs.Journaling))
    ...
    super_bloque.s_free_inodes_count -= 2
    super_bloque.s_free_blocks_count -= 2
    ...
    file.seek(start + ctypes.sizeof(structs.SuperBloque))
    for i in range(numero_estructuras):
        file.write(ctypes.string_at(ctypes.byref(journaling), ctypes.sizeof(journaling)))
    ...
    file.close()
    print("Se creo el sistema de archivos EXT3 con exito")
...
class mkfs():
    ...
    def crear_mkfs(self):
        ...
        if not mounted.is_formated:
            print("Error: La particion extendida no se puede formatear")
            return
        ...
        if self.fs == '2fs':
            make_ext2(mounted.part_s, mounted.part_start, mounted.path)
        elif self.fs == '3fs':
            make_ext3(mounted.part_s, mounted.part_start, mounted.path)
